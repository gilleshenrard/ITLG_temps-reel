...
for i in {1..100} ; do
    ...
    bin/readerswriters 98 5 30 none > stat/rw_nosched_98-5-30.txt

    #run readerswriters with nice and export the output in a temp file
    echo "with nice(), run #$i"
    bin/readerswriters 98 5 30 nice 1 10 > stat/rw_nice_98-5-30.txt

    #run readerswriters with fifo and export the output in a temp file
    echo "with FIFO, run #$i"
    bin/readerswriters 98 5 30 fifo > stat/rw_fifo_98-5-30.txt
