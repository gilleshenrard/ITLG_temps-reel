...
int threads_launch(pthread_t th_array[], thrw_t rw_array[], const uint16_t nbthreads, const uint8_t sched, const uint8_t algo);
int threads_join(pthread_t threads[], thrw_t rw_array[], const uint16_t nbthreads, const uint8_t algo);
int free_rw(thrw_t* arrays, pthread_t* threads, const uint8_t algo);
...
int main(int argc, char *argv[]){
    ...
    if(threads_launch(th_array, rw_array, nbthreads, sched, algo) < 0){
        print_error("Error while launching threads");
        exit(EXIT_FAILURE);
    }

    //wait for all the threads to finish running
    threads_join(th_array, rw_array, nbthreads, algo);

    //free memory used by all the readers/writers
    free_rw(rw_array, th_array, algo);
...
/*	   -1 otherwise																		*/
/****************************************************************************************/
int free_rw(thrw_t* array, pthread_t* threads, const uint8_t algo){
